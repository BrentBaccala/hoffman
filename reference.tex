...
This optional element specifies the format of the tablebase entries.
It has no attributes, and must contain exactly one of the following
elements:
...
\item[{\tt dtm}] Distance-to-mate.  Required to generate a tablebase
with a {\tt dtm} {\tt format}; otherwise useless and optional.  Can be
used with any number of {\tt bits} (unlike the {\tt dtm} element in
the tablebase {\tt format}, which can only have 8 or 16 {\tt bits}),
but a fatal error will result during tablebase propagation if this
field is too small.

\item[{\tt movecnt}] Required. Used to count the number of possible
moves from a given position.  Has four reserved values, and must be
able to count down to zero, so an $n$-bit {\tt movecnt} allows
positions with up to $2^n-5$ moves.  If 8-way symmetry is in use, then
many positions require their moves to be counted twice, effectively
halving that number.  If positions exist with too many possible moves
to fit into {\tt movecnt}, a fatal error will result during tablebase
initialization.
